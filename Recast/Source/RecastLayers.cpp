							  ...
							  rcHeightfieldLayerSet& lset)
{
	...
	for (int i = 0; i < lset.nlayers; ++i)
	{
		...
		const int extraHeightSize = sizeof(short) * (lw * 2 + lh * 2 + 4);
		layer->extraHeights = (short*)rcAlloc(extraHeightSize, RC_ALLOC_PERM);
		if (!layer->extraHeights)
		{
			ctx->log(RC_LOG_ERROR, "rcBuildHeightfieldLayers: Out of memory 'extraHeights' (%d).", extraHeightSize);
			return false;
		}
		memset(layer->extraHeights, 0xffff, extraHeightSize);
					...
					for (int dir = 0; dir < 4; ++dir)
					{
						if (rcGetCon(s, dir) != RC_NOT_CONNECTED)
						{
							...
							if (chf.areas[ai] != RC_NULL_AREA && i == alid)
							{
								...
								const rcCompactSpan& as = chf.spans[ai];
								if (as.y > hmin)
									th = rcMax(th, (short)((int)as.y - hmin));
							}
						}
					}
					if (bFound)
