		...
		for (int x = 0; x < xSize; ++x)
		{
			...
			for (rcSpan* span = heightfield.spans[x + z * xSize]; span != NULL; previousSpan = span, span = span->next)
			{
				...
				if (!walkable && previousWasWalkable)
				{
					...
					{
						// �����ǰ span ����һ�� span�ĸ߶Ȳ�С�ڵ��� walkableClimb����� span ���� ID ���Ϊ��һ�� span ͬһ���� ID
						span->area = previousArea;
					}
				}
				// Copy walkable flag so that it cannot propagate
				// past multiple non-walkable objects.
				// ������һ�� span ����Ϣ��todo: walkable��Ҫ���£���
	...
	for (int z = 0; z < zSize; ++z)
	{
		for (int x = 0; x < xSize; ++x)
		{
			for (rcSpan* span = heightfield.spans[x + z * xSize]; span; span = span->next)
			{
				...
				// ���������ĸ������ span
				// ���� bot ���� 0 ���������ʵ -walkableClimb - bot ����߶Ȳ��Ѿ����� minNeighborHeight > - walkableClimb �ˣ�����ֱ�� break ��
				// ���� bot С�ڵ��� 0 ���������ǰ span ���ǿ��ܻ������Ϊ������
				// bot С�ڵ���0���ǵ����ĵ�һ��
				for (int direction = 0; direction < 4; ++direction)
				{
					...
					{
						// ��������߶���Χ���� minNeighborHeight ���Ϊ -walkableClimb - bot������
					...
					{
						// ������ڳ�ʼ span �ĵײ��뵱ǰ span ������������ walkableHeight��������
						// ��ʵ�����ǲ������˸� continue ����Ϊ����ı����Ѿ�û��������
					...
					for (neighborSpan = heightfield.spans[dx + dy * xSize]; neighborSpan; neighborSpan = neighborSpan->next)
					{
						...
						if (rcMin(top, neighborTop) - rcMax(bot, neighborBot) > walkableHeight)
						{
							...
							{
								// ������� span ��������ĸ߶Ȳ�С�� walkableClimb����ȡ���ڸ��ӿ����߱������С�����߶�
				...
				{
					// ������ڸ����뵱ǰ span �ĸ߶Ȳ���� walkableClimb���� span ������ͻ�𣬲�����
				...
				{
					// ������� span �Ŀ����߱���߶Ȳ�� walkableClimb���� span ������ͻ�𣬲����� 
					span->area = RC_NULL_AREA;
				}
			}
		}
